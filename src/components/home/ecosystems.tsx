        ...
        <div className="marquee mt-8">
          <ul className="marquee-track">
            {loop.map((name, i) => (
              <li
                key={`${name}-${i}`}
                className="glass-card shrink-0 rounded-full px-4 py-2.5 text-sm text-muted grayscale transition-colors duration-200 hover:text-accent hover:grayscale-0"
              >
                {name}
              </li>
            ))}
